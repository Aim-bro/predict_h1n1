...
def _prep_categoricals(X: pd.DataFrame, cat_cols: list[str], cats_map=None):
    """
    LightGBM에서 categorical feature를 안정적으로 쓰기 위한 전처리
    - NaN -> "missing"
    - dtype -> category
    - fold 기준 train categories로 set_categories 고정
    """
    X2 = X.copy()
    ...
    for c in cat_cols:
        # 문자열/혼합 타입일 수 있으니 통일
        s = X2[c].astype("object").where(~X2[c].isna(), "missing")

        if c not in cats_map:
            s = s.astype("category")
            cats_map[c] = s.cat.categories
        else:
            s = pd.Categorical(s, categories=cats_map[c])
...
def cv_f1_with_inner_threshold_lgbm(
    df: pd.DataFrame,
    target: str,
    n_splits: int = 5,
    seed: int = 42,
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(X, y), start=1):
        ...
        # inner split (threshold + early stopping 모두 train fold 내부에서)
        ...
        # categorical prep (train fold 기준 categories 고정)
        X_tr2_p, cats_map = _prep_categoricals(X_tr2, cat_cols, cats_map=None)
        X_tune_p, _ = _prep_categoricals(X_tune, cat_cols, cats_map=cats_map)
        X_va_p, _ = _prep_categoricals(X_va, cat_cols, cats_map=cats_map)

        # class imbalance 반영
        pos = float(y_tr2.sum())
        neg = float(len(y_tr2) - y_tr2.sum())
